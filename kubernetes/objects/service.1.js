...
// const TENX_SCHEMA_PORTNAME = 'paas.enncloud.cn/schemaPortname'
const TENX_SCHEMA_PORTNAME = 'tenxcloud.com/schemaPortname'
const TENX_SCHEMA_LBGROUP = 'system/lbgroup'

class Service {
    constructor(name, cluster,type,serviceName) {
            this.kind = 'Service'
            this.apiVersion = 'v1'
            ...
            if (cluster && cluster.publicIPs && type==1 ) {
    ...
    addPort2(proxyType, name, protocol, targetPort, port, nodePort) {
        // K8s only support TCP and UDP protocol
        const k8sProtocol = (protocol === 'UDP' ? protocol : 'TCP')
        const portObj = {
            name,
    ...
    addLBGroupAnnotation(groupName,type,accessMethod) {
        if (!this.metadata.annotations) {
            this.metadata.annotations = {}
        }
        if(type=='1'){
            // this.metadata.annotations[TENX_SCHEMA_LBGROUP] = groupName
            this.metadata.annotations[TENX_SCHEMA_LBGROUP] = accessMethod
        }
    }
}
