...
// import { loadStateDetail, deleteStateData, putStateDetail, loadDbCacheList } from '../../actions/database_cache'
import { loadStateDetail, deleteStateData, putStateDetail, loadDbCacheList } from '../../../actions/database_cache'

import { setServiceProxyGroup } from '../../../actions/services'
import { getCookie, setCookie,formatDate } from '../../../common/tools'
import { getProxy } from '../../../actions/cluster'
import '../../../components/AppModule/style/ModalDetail.less'
// import './style/ModalDetail.less'
// import AppServiceEvent from './../AppServiceDetail/AppServiceEvent'
import AppServiceEvent from './../../../components/AppModule/AppServiceDetail/AppServiceEvent'
import PreviewConfig from './../../../components/AppModule/PreviewConfig'
    ...
    return (
      <div className='modalDetailBox' id="dbClusterDetailInfo">
        <div className='configContent'>
          <div><div className='configHead'>基本信息</div>
            <ul className='parse-list'>
              <li><span className='key'>名称</span> <span className='value'>{databaseInfo.config.objectMeta.name}</span></li>
              <li><span className='key'>镜像名称</span> <span className='value'>{ databaseInfo.containerImages[0] }</span></li>
              <li><span className='key'>创建时间</span> <span className='value'>{formatDate(databaseInfo.objectMeta.creationTimestamp)}</span></li>
            </ul>
          </div>
...
class VisitTypes extends Component{
  ...
  componentWillMount() {
    ...
    let nameSpace = getCookie('tce_user_current_config').split(',')[1]
  ...
  onChange(e) {
    ...
    if (value === 1) {
      ...
      flag = 'other'
      this.setState({
        'initGroupID':this.state.proxyArr[0].id
      })
    }
  }
  toggleDisabled() {
      ...
      setServiceProxyGroup({
        ...
        success: {
          func: (res) => {
            ...
            const { loadStateDetail } = scope.props
            setTimeout(() => {
              loadStateDetail(clusterID, databaseInfo.objectMeta.name, false,{},this.props.database)
            }, 0)
            this.setState({
              disabled: true,
              forEdit:false
  ...
  render() {
    ...
    const proxyNode = proxyArr.length > 0 ? proxyArr.map((item,index)=>{
      return (<Option defaultValue={item.id} key={item.id} value={item.id}>{item.type == 'public' ? '公网：' : '内网：'}{item.name}</Option>)
    }):null
    let domain = ''
    if (domainSuffix) {
      ...
      <Card id="visitsTypePage">
          ...
          <div className="visitTypeInnerBox">
            ...
            <div className="radioBox">
              ...
              <p className="typeHint">
                {
                  radioValue === 1 ? '选择后该应用仅提供集群内访问；': '应用可提供集群外访问，选择一个网络出口；'
                }
              </p>
        ...
        <div className="visitTypeBottomBox">
          ...
          <div className="visitAddrInnerBox">
            ...
            {databaseInfo.serviceInfo.annotations['system/lbgroup']!='none'?
...
class ModalDetail extends Component {
  ...
  deleteDatebaseCluster(dbName) {
    ...
    const { deleteStateData, cluster, scope, database,clusterType } = this.props;
    const { loadStateDetail } = scope.props;
    const _this = this
    ...
    deleteStateData(cluster, dbName, 'appstore', {
      success: {
        func: () => {
  ...
  componentWillMount() {
    const { loadStateDetail, cluster, dbName } = this.props
    const _this = this
    this.setState({
      currentDatabase: dbName,
    });
    loadStateDetail(cluster, dbName,true, {
      success: {
        func: (res) => {
          _this.setState({
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const { loadStateDetail, cluster, dbName } = nextProps;
    const _this = this
    if (dbName != this.state.currentDatabase) {
      ...
      loadStateDetail(cluster, nextProps.dbName,true, {
        success: {
          func: (res) => {
  ...
  handSave() {
    const {putStateDetail, cluster, dbName, loadDbCacheList, loadStateDetail} = this.props
    const parentScope = this.props.scope
    const _this = this
    const notification = new NotificationHandler()

    this.setState({putModaling: true})
    putStateDetail(cluster, dbName, this.state.replicas, {
      success: {
        func: (res) => {
          notification.success('更新成功')
          parentScope.setState({ detailModal: false,putVisible:false })
          _this.setState({putModaling: false})
          setTimeout(function(){
            loadDbCacheList(cluster,'appstore')            
          },1000)
          loadStateDetail(cluster, dbName,true, {
            success: {
              func: (res) => {
  ...
  render() {
    const {form, scope, dbName, isFetching, databaseInfo, domainSuffix, bindingIPs } = this.props;
        ...
        <div className='topBox'>
          ...
          <div className='infoBox'>
            <p className='instanceName'>
              {databaseInfo.objectMeta.annotations['pod.alpha.kubernetes.io/appname']!=""?databaseInfo.objectMeta.annotations['pod.alpha.kubernetes.io/appname']:databaseInfo.serviceInfo.name}
            </p>
            <div className='leftBox TenxStatus'>
              <div className="desc">{databaseInfo.serviceInfo.namespace} / {databaseInfo.objectMeta.annotations['pod.alpha.kubernetes.io/appname']!=""?databaseInfo.objectMeta.annotations['pod.alpha.kubernetes.io/appname']:databaseInfo.serviceInfo.name}</div>
              <div> 状态：
        ...
        <div className='bottomBox'>
          <div className='siderBox'>
            <Tabs
              ...
              {/* <TabPane tab='模板参数' key='#config'>
                <PreviewConfig scope={this} cluster={this.props.cluster} database = {databaseInfo} config = {databaseInfo.config}/>
              </TabPane> */}
            </Tabs>
          </div>
        </div>
...
function mapStateToProps(state, props) {
  ...
  const { databaseStateDetail } = state.databaseCache
  const { databaseInfo, isFetching } = databaseStateDetail.databaseInfo || defaultMysqlList
  return {
    isFetching,
    cluster: cluster.clusterID,
...
ModalDetail.PropTypes = {
  ...
  loadStateDetail: PropTypes.func.isRequired,
  deleteStateData: PropTypes.func.isRequired,
}
