...
import { loadStateDetail, deleteStateData, putStateDetail, loadStatefulsets } from '../../actions/database_cache'
        ...
        <div className='configContent'>
          <div><div className='configHead'>基本信息</div>
            <ul className='parse-list'>
              ...
              <li><span className='key'>创建时间</span> <span className='value'>{ calcuDate(databaseInfo.config.objectMeta.creationTimestamp) }</span></li>
            </ul>
          </div>
...
class VisitTypes extends Component{
      ...
      setServiceProxyGroup({
        ...
        success: {
          func: (res) => {
            ...
            setTimeout(() => {
              loadStateDetail(clusterID, databaseInfo.objectMeta.name, false)
            }, 0)
            this.setState({
              disabled: true,
              forEdit:false
  ...
  render() {
    ...
    const proxyNode = proxyArr.length > 0 ? proxyArr.map((item,index)=>{
      return (<Option key={item.id} value={item.id}>{item.type == 'public' ? '公网：' : '内网：'}{item.name}</Option>)
    }):null
    let domain = ''
    if (domainSuffix) {
...
class ModalDetail extends Component {
  ...
  deleteDatebaseCluster(dbName) {
    ...
    const { deleteStateData, cluster, scope, database } = this.props;
    const { loadStateDetail } = scope.props;
    const _this = this
  ...
  componentWillMount() {
    ...
    loadStateDetail(cluster, dbName, {
      success: {
        func: (res) => {
          _this.setState({
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (dbName != this.state.currentDatabase) {
      ...
      loadStateDetail(cluster, nextProps.dbName, {
        success: {
          func: (res) => {
  ...
  handSave() {
    const {putStateDetail, cluster, dbName, loadStatefulsets, loadStateDetail} = this.props
    const parentScope = this.props.scope
    const _this = this
    const notification = new NotificationHandler()
      ...
      <div id='AppServiceDetail' className="dbServiceDetail">
        <div className='topBox'>
          ...
          <div className='imgBox'>
            <img src={databaseInfo.objectMeta.annotations.imageUrl} width="120" height="120" />
          </div>
