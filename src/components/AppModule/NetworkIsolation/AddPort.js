...
class AddFrom extends Component{
    ...
    addFromsKey=()=>{
        ...
        FromKeys.forEach(key => {
            ...
            validateFieldsKeysFrom.push(`port${keyValue}`)
            validateFieldsKeysFrom.push(`portProtocol${keyValue}`)
            const portProtocolValue = getFieldValue(`portProtocol${keyValue}`)

            if (portProtocolValue === 'TCP' && accessMethod !== 'Cluster') {
                validateFieldsKeysFrom.push(`mappingPortType${keyValue}`)
                const mappingPortTypeValue = getFieldValue(`mappingPortType${keyValue}`)
                if (mappingPortTypeValue === MAPPING_PORT_SPECIAL) {
                    validateFieldsKeysFrom.push(`mappingPort${keyValue}`)
                }
            }
        })
        validateFields(validateFieldsKeysFrom, (errors, values) => {
            if (!!errors) {
                return
            }
