...
import Deployment from '../../../../kubernetes/objects/PetSet'
import Service from '../../../../kubernetes/objects/service.1'
...
export function getFieldsValues2(fields) {
  const values = {}
...
export function genRandomString2(mytoken, len){
...
export function buildJson2(fields, cluster, loginUser, imageConfigs,current,stackDetail) {
  const fieldsValues = getFieldsValues2(fields)
  // 获取各字段值
  const {
    appName,
    serviceName, // 服务名称
  ...
  const deployment = new Deployment(serviceName,current.space.namespace,stackDetail.appDetail,storageSize,appName,item,current)
  // 设置镜像地址
  deployment.addContainer(serviceName,`${imageUrl}:${imageTag}`, mountPaths?mountPaths:'',stackDetail.appDetail,storageSize,current)
  // 设置绑定节点
  ...
  // 设置端口
  const service = new Service(serviceName+'-'+genRandomString2(5), cluster,1,serviceName)
  const service1 = new Service(serviceName, cluster,0,serviceName)
  const { proxyType } = loginUser

  let groupID = publicNetwork || internaletwork || "none"
  service.addLBGroupAnnotation(serviceName+'/TCP',1,groupID)
  service1.addLBGroupAnnotation(serviceName+'/TCP',0)

  const appPorts = JSON.parse(stackDetail.appDetail.ports)
  appPorts && appPorts.forEach((key,index) => {
    if (key.deleted) {
      return
    }
    const keyValue = index//key.value
    const name = `${serviceName}-${keyValue}`
    const port = Number(key.split('/')[0])
    const portProtocol = key.split('/')[1]
    const portType = key.split('/')[2]
    ...
    if(key.split('/')[2]==1){
      ...
      service1.addPort(proxyType, name, portProtocol, port, port, mappingPort)
    }
  })
  ...
  if (envKeys) {
    ...
    deployment.addContainerEnv(serviceName, "APP_NAME", appName)
    deployment.addContainerEnv(serviceName, "SERVICE_NAME",serviceName)
  }
  ...
  return { deployment, service,service1 }
}