...
let MyComponent = React.createClass({
  ...
  renderBuildBtn(item, index) {
    ...
    if (!branchesAndTags) {
      if (stagesCount > 0) {
        tabs.push(<PopOption key="not_found_branches_tags">未找到分支及标</PopOption>)
      }
    } else {
        ...
        <div className='tenxflowDetail' key={item.name} >
          <div className='name'>
            <Tooltip placement='topLeft' title={item.name}>
              <Link to={`/ci_cd/build_image/enn_flow_build?${item.flowId}&${flowListState[index].status}`}>
                <span>{item.name}</span>
              </Link>
            </Tooltip>
          </div>
